...
def task_test_settings() -> Settings:
    return Settings(
        APP_NAME="DevFlow-TaskTest",
...
@pytest.fixture(scope="module")
def task_test_app(task_test_settings: Settings):
    get_settings.cache_clear()
    return create_app(settings=task_test_settings)
...
async def client(task_test_app, db_session):
    ...
    task_test_app.dependency_overrides[get_redis_client] = override_get_redis
    task_test_app.dependency_overrides[get_db] = override_get_db

    async with AsyncClient(
        transport=ASGITransport(app=task_test_app),
        base_url="http://testclient",
    ) as ac:
        yield ac
        
    task_test_app.dependency_overrides.clear()
...
async def _create_task(client: AsyncClient, headers: dict, project_id: str, title: str | None = None) -> dict:
    uid = uuid.uuid4().hex[:6]
    payload = {"title": title or f"Test Task {uid}", "status": "todo", "priority": "medium"}
...
async def test_create_task(client: AsyncClient):
    ...
    assert task["title"].startswith("Test Task")
    assert "id" in task
...
async def test_list_tasks(client: AsyncClient):
    ...
    await _create_task(client, alice["headers"], project["id"])
    await _create_task(client, alice["headers"], project["id"])
    
    resp = await client.get(f"/api/v1/projects/{project['id']}/tasks", headers=alice["headers"])
...
@pytest.mark.asyncio
async def test_cross_org_task_access_403(client: AsyncClient):
    alice = await _register_and_login(client)
    ...
    resp = await client.get(f"/api/v1/tasks/{task['id']}", headers=bob["headers"])
